...
function wbInjectPanel(){
  ...
  <div style="display:flex;border-bottom:1px solid rgba(160,80,220,.15);flex-shrink:0">
    ...
    <button class="wb-tab" data-tab="wb-tab-links"
      style="flex:1;padding:7px 4px;font-family:'Space Mono',monospace;
             font-size:var(--fs-sm);background:none;border:none;
             color:var(--pdm);cursor:pointer;border-bottom:2px solid transparent">Links</button>
  </div>
  ...
  <div style="overflow-y:auto;flex:1;padding:12px 14px">
    ...
    <!-- ── TAB: Links ────────────────────────────────────────────────── -->
    <div id="wb-tab-links" class="wb-tab-content" style="display:none">
      <div style="font-size:var(--fs-sm);color:var(--pdm);margin-bottom:10px">
        Faction and civilization connections from this star to others.
        Lines are drawn in the galactic map when Faction Lines is toggled on.
      </div>
      <div id="wb-conn-list" style="margin-bottom:8px"></div>
      <div id="wb-conn-form" style="border:1px solid rgba(100,150,255,.2);border-radius:8px;padding:8px;display:none">
        <div style="font-size:var(--fs-sm);color:#88aaff;margin-bottom:6px;letter-spacing:.06em">ADD CONNECTION</div>
        <div class="wb-field"><label class="wb-lbl">Era</label>
          <select id="wb-conn-era" class="wb-inp"></select></div>
        <div class="wb-field"><label class="wb-lbl">Faction</label>
          <select id="wb-conn-faction" class="wb-inp"></select></div>
        <div class="wb-field"><label class="wb-lbl">Type</label>
          <select id="wb-conn-type" class="wb-inp">
            <option>Territory</option>
            <option>Trade Route</option>
            <option>Military</option>
            <option>Other</option>
          </select></div>
        <div class="wb-field"><label class="wb-lbl">Connect to Star</label>
          <input id="wb-conn-search" class="wb-inp" type="text" placeholder="Type star name…" autocomplete="off">
          <div id="wb-conn-results" style="display:none;background:var(--pn);border:1px solid rgba(0,0,0,.1);
            border-radius:6px;margin-top:2px;max-height:120px;overflow-y:auto;font-size:var(--fs-sm)"></div>
          <input type="hidden" id="wb-conn-hip-b" value="">
          <div id="wb-conn-selected" style="font-size:var(--fs-sm);color:#44aa66;margin-top:3px"></div>
        </div>
        <div class="wb-field"><label class="wb-lbl">Notes</label>
          <input id="wb-conn-notes" class="wb-inp" type="text" placeholder="Optional…"></div>
        <div style="display:flex;gap:6px;margin-top:6px">
          <button id="wb-conn-save" class="wb-btn-primary" style="flex:1">✓ Save Connection</button>
          <button id="wb-conn-cancel" class="wb-btn" style="flex:0 0 auto">Cancel</button>
        </div>
      </div>
      <button id="wb-conn-add-btn" class="wb-btn" style="width:100%;margin-top:4px;font-size:var(--fs-sm)">
        + Add Connection
      </button>
    </div>
